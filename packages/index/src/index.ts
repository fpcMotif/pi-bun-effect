...
export class InMemorySessionIndex implements SearchIndex {
  ...
  async querySessions(
    ...
    for (const row of this.rows) {
      if (row.text.includes(normalized)) {
        scores.set(row.sessionId, (scores.get(row.sessionId) ?? 0) + 1);
      }
    }
    ...
    return [...scores.entries()]
      ...
      .sort((a, b) =>
        b.score - a.score || a.sessionId.localeCompare(b.sessionId)
      )
      .slice(0, limit);
  }
...
export class SqliteSessionIndex implements SearchIndex {
  ...
  async upsertSession(
    ...
    ).run(sessionId, entryId, normalized, now);
  ...
  async querySessions(
    ...
    return db
      .query(
        `SELECT session_id AS sessionId, COUNT(*) AS score
...
export async function createSessionIndex(
  ...
  if (requested === "sqlite") {
    if (await canUseBunSqlite()) {
      return new SqliteSessionIndex();
    }
    throw new Error("sqlite backend requested but bun:sqlite is not available");
  }

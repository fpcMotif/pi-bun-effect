...
export function resolveSafePath(requestedPath: string, root?: string): string {
  const base = root ? resolve(root) : process.cwd();
  const resolvedPath = resolve(base, requestedPath);
...
const READ_TOOL: ToolDefinition = {
  ...
  async run(context, invocation) {
    try {
      ...
      const resolved = resolveSafePath(path, context.sandboxRoot);
      ...
      if (typeof Bun === "undefined") {
        return {
          content: {
            ...
            toolCallId: `tool-${randomUUID()}`,
            toolName: "grep",
            content: [{ type: "text", text: `mock-grep:${pattern}:${resolved}` }],
          },
        };
      }
      ...
      if (typeof Bun === "undefined") {
        return {
          content: {
            ...
            toolCallId: `tool-${randomUUID()}`,
            toolName: "find",
            content: [{ type: "text", text: `mock-find:${pattern}:${resolved}` }],
          },
        };
      }

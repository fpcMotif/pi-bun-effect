...
test("denyPatterns are isolated per extension", async () => {
  ...
  expect(resultA.reason).toContain(
    "Blocked by extension safety pattern: forbidden-a",
  );

  const resultA2 = await engine.check(
    "ext-a",
    "exec:spawn",
    "echo forbidden-b",
  );
  ...
  expect(resultB.reason).toContain(
    "Blocked by extension safety pattern: forbidden-b",
  );

  const resultB2 = await engine.check(
    "ext-b",
    "exec:spawn",
    "echo forbidden-a",
  );
...
test("default denyPatterns still apply to all extensions", async () => {
  const engine = createPolicyEngine();
...
test("denyCommands matching is case-insensitive", async () => {
  const engine = createPolicyEngine([
    {
      extensionId: "test-ext",
      capabilities: ["exec:spawn" as Capability],
  ...
  const result = await engine.check("test-ext", "exec:spawn", "REBOOT");
  expect(result.allowed).toBeFalse();
  expect(result.reason).toBe("Command denied by denylist");
});
